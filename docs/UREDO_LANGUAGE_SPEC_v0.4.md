...
- **Revision 2026-09-15f (v0.4):** **§36's position measured rather than asserted.** `examples/restdemo` and its idiomatic Rust twin were benchmarked against each other on one machine under one load generator — `GET /items/1`, 32 keep-alive connections, 8-second runs, five repetitions, server and generator pinned to separate halves of the machine — and came out at **39,495 against 39,219 requests per second, +0.7% on medians**, with p50 latencies of about 740 and 750 microseconds. Uredo won four of the five paired runs, which at that margin is a coin flip. The control is what makes the null reportable: the **same binary** run twice varies by **25.4%**, and even the tighter within-side spreads are 7-9%, so a 0.7% difference sits far inside both. §36 says generated Rust performs like hand-written Rust and retired the runtime budget on that basis; this is the first time the claim has been checked against a running program rather than argued from the lowering. The load generator is committed (`examples/restdemo/bench/`) rather than taken off the shelf, so the figure is reproducible here and the generator's own ceiling is visible beside what it measures; it reports a distribution because two servers within noise of each other is the expected result and a mean would hide it. No claim is made against any framework: the design note permits one such comparison, narrowly stated, and it has not been run.
