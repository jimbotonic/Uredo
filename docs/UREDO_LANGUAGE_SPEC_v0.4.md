...
- **Revision 2026-09-13n (v0.4):** `uredo fix` and `uredo lsp` implemented, so **every command §28.1 names now exists** (§0.1). Two carry a stated limit rather than a gap, and saying which is the point of the row.
  ...
  **`lsp` shares the compiler's front end rather than a second one**, which is the property §28.2 asks for: the diagnostics an editor shows are the ones `uredo check` prints, on the same Uredo lines, and hover is what `uredo explain` says about that line. It speaks `initialize`, `shutdown`/`exit`, full-text sync, `publishDiagnostics` for errors, warnings and lints, `textDocument/formatting`, `textDocument/hover`, `textDocument/documentSymbol`, `textDocument/definition` and `textDocument/completion`, and answers every request carrying an `id` even when the answer is nothing, since one left unanswered hangs the editor. Thirteen tests drive it the way an editor does — `Content-Length` framing over stdio, not the functions behind it — including the two exit codes the protocol distinguishes. The outline, definition and completion are built on the *parser* rather than on a full compile, because a file mid-edit usually does not compile and an outline that empties itself on every keystroke is worse than none; the parser recovers at item boundaries, so every item but the broken one is still answered for. Definition is item-level and crosses into sibling files, since §20.3 makes a file a module and the directory is therefore the module list; it declines on a local binding rather than guessing which one. Completion offers the items in scope and the keywords, and **nothing at all after `.` or `::`**, where the useful answer is a member of whatever is on the left and there is no type engine to ask (§4.4). **What it does not have is the *lossless* half of §28.2**: the parser keeps no trivia, so it cannot reconstruct the source it was given, which is what a rename or a code action needs. That is the next thing an editor user would notice, and it is written down rather than discovered.
...
- **Revision 2026-09-15e (v0.4):** **the rule that caused four of the demo's thirteen authoring errors had no diagnostic at all.** `throws` names what a function returns when it *fails* and says nothing about what it returns when it succeeds, and that split was got wrong four times — once at seven sites in one function. All four lowered silently: writing `Ok(1)` in a `throws` body produced `Result::Ok(Ok(1))`, and what the author saw was rustc's `expected u32, found Result<…>` pointing at the **signature line**, never naming §15.3's wrapping. Two of the four are decidable from the declaration alone and are now errors in Uredo's own terms: a tail that is syntactically `Ok(…)`/`Err(…)` in a `throws` body whose value type is not itself a `Result` or optional, and a `-> T?` whose tail builds a bare `T` — a struct literal or a unit-struct path — when an optional return is verbatim and wrapped for you nowhere. Both name the rule and say what to write instead. The guard is what keeps them honest: a `Result`-valued `throws` really does want `Ok(…)` as its tail, an `Option`-yielding expression is a correct optional tail, and `None` is a path that must not be mistaken for the payload — each has a test asserting the diagnostic does **not** fire, and the whole repository lints clean. The other two of the four — `throws` with no `->` being a unit return, and a tail that is already a `Result` needing `?` — need more than the declaration and are documented in the manual's §17 rather than diagnosed. 179 tests.

- **Revision 2026-09-15f (v0.4):** **§36's position measured, after a first attempt that measured nothing at all.** `examples/restdemo` and its idiomatic Rust twin were benchmarked against each other — `GET /items/1`, 32 keep-alive connections, 8-second runs, eight repetitions, server and generator pinned to separate halves of the machine, order reversed every repetition, session warm-up discarded — and came out at **+2.9% for Uredo as the median of the per-pair differences, over a per-pair range of −33% to +10%, with 6 of 8 paired runs**. Against a same-binary control that varies by **31%**, that is parity to the resolution this machine allows, which is about ±10%. The two binaries differ by 152 bytes and carry identical release profiles, which is the independent reason to disbelieve any gap the measurement might have shown. **The first attempt reported 39,495 against 39,219 requests per second and was of neither program:** port 8080 was held by an unrelated admin service, both servers failed to bind with `Address already in use`, and the load generator measured that service twice, serving 404s to a path it did not have. The script had no check that the thing answering was the thing under test — a gate that can silently skip is not a gate, and this one skipped silently in both directions. It now picks a free port, both servers take a listen address, and it refuses to time anything until `/health` answers and the path under test returns 200. A second error was ordering: `ABAB` against a machine that ramps monotonically across a session gave the later slot to the second binary every time, which produced a confident **−12.3%** that was entirely the ordering; it is `ABBA` now, and the statistic is the median of per-pair differences rather than a difference of pooled medians. No claim is made against any framework.

- **Revision 2026-09-16 (v0.4):** **a `@derive(Copy)` type used across a module boundary did not compile, and nothing had ever tried it.** `prepass` records each function's parameter modes, and a mode depends on whether its type is known-`Copy` (§10.1) — which for an imported type is only known once the `use` has been resolved against the crate index. `prepass` ran *first*, so a `Copy` type from another module was recorded P3 at prepass and lowered P1 in the signature: the declaration took it by value and every call site in the same file inserted a borrow. `resolve_uses` now runs before `prepass`, which is the whole fix; a local declaration still shadows an import because `prepass` uses `insert`. Found by adding content negotiation to `examples/restdemo`, whose `Wire` and `Coding` enums are the first `@derive(Copy)` types this project has used from another module — the same shape of gap as the four before it, a path that existed and nothing had executed. The demo gained two ways to send less, both opt-in and both measured on the running server: a positional shape that drops the field names (`Accept: application/vnd.restdemo.compact+json`), taking one item from 117 bytes to 56, and brotli above a 256-byte threshold, taking a fifty-item listing from 5,983 to 258 — 186 with both. They matter at opposite ends, since a single item is too small for brotli to touch and a listing compresses to 4% of itself precisely because the repeated field names are what a compressor eats. Each representation carries its own ETag over the bytes actually sent, with `Vary: accept, accept-encoding`, because a cache keyed on the URL alone would hand one client's shape to another. The linter caught one idiom slip in the new code (`&Vec<String>` for `[String]`). The twin was then ported, so the §37 comparison is whole-library again and reads **tokens −9.9%, lines −18.7%, characters −3.8%, identifiers −0.7%, punctuation −17.1%**, with 57 functions on each side. The equivalence is now byte-level rather than behavioural: driven with the same requests both servers return the same lengths *and the same ETag*, which is a hash of the bytes they wrote. 179 tests, 11 on each side of the twin.

- **Revision 2026-09-16b (v0.4):** **the demo's throughput, measured against a floor rather than in the abstract.** A benchmark number for a service means nothing without knowing what the harness can do at all, and here that mattered: at 32 connections `/health` measured 166,332 requests per second and a hyper server returning a fixed body measured 165,567 — **the service was indistinguishable from one that does no work**, so the figure was the generator's ceiling rather than the service's. At 128 connections, where there is headroom, the floor is 221,638 and the service reads **204,751 on `/health` (92% of floor), 187,923 on `/items/1` (84%) and 89,252 on a fifty-item listing (40%)**. So the routing, the locked store read, the serialisation and the ETag hash together cost about 16% of what this machine and this HTTP stack can do; the rest is hyper, loopback and the generator. `examples/restdemo/bench/src/bin/floor.rs` is committed alongside the generator for that reason. **No table is published against Rapidoid or the TechEmpower figures**, and the demo's design note already forbade one: the published numbers come from server-class hardware with the client on a separate machine, the large ones are the *pipelined* plaintext test against a fixed thirteen-byte body, and this generator does not pipeline while `/items/1` takes a read lock, looks up a map, clones, serialises and hashes. Any one of those moves throughput by more than the difference anyone would be comparing. The honest route to a cross-framework number is to run this service inside the other harness under its rules, not to set two numbers taken under different conditions beside each other.

- **Revision 2026-09-16c (v0.4):** **a TechEmpower front-runner built and run on this machine, which is the only cross-framework comparison that transfers.** `may-minihttp` was fetched from the benchmark's own repository, built, and run against a real Postgres carrying TechEmpower's schema, so its code path is upstream; only the database host and the listen address differ, because `tfb-database` is a name that exists only inside their harness. Under the same generator at 128 connections: `may-minihttp` `/plaintext` **353,986**, `/json` **257,625**, `uredo` `/health` **216,904**, a bare hyper server returning a fixed body **196,390**, and `uredo` `/items/1` **194,919**. Over three repetitions with the order reversed each time, `/health` measured 75%, 76% and 99% of `/json` — median about **76%** on comparable work. Two readings come out of it, and the second is the one that matters: **the gap is mostly hyper rather than Uredo**, since `may-minihttp` beats a bare hyper server by 31% by not using hyper at all — it carries its own HTTP implementation on coroutines, so anything built on hyper in any language is bounded near that floor; and **`uredo /items/1` matches bare hyper while taking a read lock, looking up a map, cloning, serialising eight fields and hashing an ETag**. `examples/restdemo/bench/against-tfb.sh` rebuilds the whole setup so it can be re-run rather than believed. Checked against the source rather than recalled: the TechEmpower repository is **archived**, with Round 23 its last, so its published figures are historical and a current number can only be taken locally.

- **Revision 2026-09-16d (v0.4):** **an HTTP server on stackful coroutines, written in Uredo, reaches parity with the TechEmpower front-runner.** `examples/restdemo/coroutine` is sixteen lines on `may_minihttp` — the front-runner's own crate — and over three alternating repetitions at 128 connections it measured **368,557 requests per second against `may-minihttp`'s 361,867 on comparable work, winning two of three**, which at a 2% margin is a coin flip. The same service on hyper measured 287,611, so the coroutine stack is worth about **28%** and the surface syntax is worth nothing either way: the generated Rust is the same work, so swapping the HTTP stack moves the number and swapping the language does not. It is §36's position reached from a third direction, after the twin benchmark and the floor. Two things the spike surfaced about the language rather than the stack. A coroutine handler is straight-line blocking code, so it needs **no `async fn`, no `.await` and no futures** — and `async` blocks are the one construct that still requires `rust { }` (§38, held with D51), so the tokio version of this service contains one and the coroutine version contains none. And `fn call(self: inout, req: take Request, …)` is the reminder that **a foreign trait fixes its own parameter modes**: D53's table covers std traits, so for anyone else's trait the mode is written, and §10.2's default would have borrowed a parameter the trait passes by value. The spike is one endpoint, not the service, and `may`'s stackful coroutines carry their own costs — `unsafe` underneath, care around blocking syscalls and thread-local storage — so the 28% is bought rather than found.

- **Revision 2026-09-16e (v0.4):** **the full read path ported to the coroutine edge, a prediction that was wrong, and a seventh compiler defect.** `examples/restdemo/coroutine` now serves `/items/1` with the same lock, lookup, clone, eight-field serialisation and ETag as the tokio edge — *the same code*, since the store, model, query parser, representations, ETag and router come from the service's own library as a dependency and only the HTTP edge is written twice. The prediction, stated before measuring, was that the 28% seen on `/health` would **shrink**, because `/items/1` spends more of its time in code the stack does not touch. It **grew**, to +48%, +47% and +43% over three alternating repetitions. The likely reason, offered as hypothesis: the hyper edge builds a `Response` through a builder with a `HeaderMap` and copies the body into `Bytes`, while the coroutine edge pushes headers into a fixed array and moves the `Vec` — so the more a response carries, the more the hyper edge pays. One asymmetry is ours rather than the stacks': on the hyper edge `/health` sets one header and `/items/1` sets four, while the coroutine edge sets four for both. **What the faster stack costs is worth recording next to what it saves:** `may_minihttp` 0.1.11 — the version the TechEmpower entry itself depends on — **cannot set a header computed at runtime**, its `header()` taking `&'static str`, so an ETag cannot be expressed at all and this port depends on the unreleased master. Part of the speed is bought by doing less. **The defect:** `uredo fmt` spaced the angle brackets of a generic inside a *tuple* type — `-> (u16, Vec<u8>)` became `-> (u16, Vec < u8 >)` — because a `(` ended the type context and the commas inside it kept it ended. It went unnoticed because the result is still valid Rust: `corpus/10_iterators` had carried `(Vec < u32 >, Vec < u32 >)` in its committed source, and the fix corrects it with the token count unchanged. `compiler/tests/fixtures/fmt_hard/src/tuple_type.ure` keeps the shape. Also found: a path dependency on another Uredo package must name its *generated* crate, since Cargo cannot build a package whose `src/` holds `.ure` files. 179 tests.
...
**`uredo lsp` shares the compiler's front end**, so the diagnostics an editor shows are the diagnostics `uredo check` prints, on the same Uredo lines, and hover is what `uredo explain` says about that line (§26.2). It speaks `initialize`, `shutdown`/`exit`, full-text sync, `publishDiagnostics` (errors, warnings and lints), `textDocument/formatting`, `textDocument/hover`, `textDocument/documentSymbol`, `textDocument/definition` and `textDocument/completion`; every request with an `id` is answered, including one it does not support, since a request left unanswered hangs the editor. On a file that does not parse it answers from what still did. The parser recovers at item and statement boundaries — a broken line costs its own statement, a broken header costs its own item and no more — so `compile_tolerant` lowers the rest and hover answers from it, saying that the file has an error elsewhere. Only when the broken item is the one being asked about does the server fall back to the last text that compiled, and then only for a line unchanged since. The outline (`documentSymbol`), jump-to-definition and completion are built on the parser directly rather than on `compile_tolerant`, for the same reason and one more: they need no lowering, so they answer on a file that has never compiled at all. Definition is item-level — a function, a type, a constant, a field, a variant, a module — and searches the sibling `.ure` files when the open one does not declare the name, because §20.3 makes a file a module and the directory is the module list; on a local binding it answers nothing rather than the wrong `x`. Completion offers the items in scope and the keywords, and refuses after `.` or `::`: the right answer there is a member of the thing on the left, Uredo has no type engine to ask (§4.4), and offering every top-level name instead is how a completion list teaches people to stop reading it. What is still owed is the *lossless* half: the parser keeps no trivia and no error nodes, so it cannot reconstruct the source it was given, which is what a rename or a code action would need.
