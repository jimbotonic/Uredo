...
| `compiler/` | The reference compiler and CLI, in Rust. `cargo test` runs 192 tests. |
| `corpus/` | Twenty paired programs — Uredo and idiomatic Rust — that must print the same thing. `run.py` measures them; `perf.py` measures allocation, memory and binary size; `TOKENS.md` answers *how much does it save, and on what does that depend* across every pair this repository can measure, rather than with one number. |
...
- The language server's parser is **tolerant but not lossless**. Tolerant is why diagnostics,
  hover, the outline, jump-to-definition and completion all keep working on a file mid-edit: the
  parser recovers at item boundaries, so every item but the broken one is still there. Lossless is
  what it does not have — no trivia is kept, so the source cannot be reconstructed, which is what
  **rename and code actions** would need, and neither is offered.
...
**How many tokens it saves, and on what that depends.** A single figure invites the question
*which code?* and cannot answer it, so the answer is a distribution and a rule
([`corpus/TOKENS.md`](corpus/TOKENS.md), reproduced by `corpus/tokens.py`):

- **−3.8% to −18.5%, median −13.0%**, over 29 measured pairs — the twenty corpus programs, six
  pieces of real Rust round-tripped from other repositories, and three cuts of the REST service.
  The gated corpus figure, −13.4% on the application-style programs, is not wrong and is not
  typical: it sits near the top, because those programs were written to exercise language features
  and are dense in exactly what Uredo removes. (The v0.1 draft's 28.7% was a punctuation-dense best
  case and was never the target.)
- **Three quarters of everything removed is braces and semicolons.** The rest nearly cancels:
  `let`, call-site `&`, closure pipes and attribute brackets come off; the colon ending every block
  header and carrying every type annotation goes back on, and is the largest single cost. So the
  saving is proportional to how many **statements and blocks** the code has per token, and to
  nothing else — brace density is the only predictor with signal (r = +0.52); `::`, `<>`, `&` and
  identifier shares all correlate at |r| < 0.25.
- **Which means code dense in paths, generics or long call chains lands at the bottom** — the REST
  service's HTTP edge is −6.6% and its axum edge −5.6%, both spending over 5.8% of their tokens on
  `::` alone — **and code inside `rust { }` saves exactly nothing**, by construction. That last
  clause is the escape hatch working, not a caveat: a figure that hid it would be measuring the
  wrong thing.

Measured on the twenty-program corpus and its idiomatic Rust twins:
