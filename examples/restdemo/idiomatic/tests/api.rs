//! The service, exercised over a real socket. The same checks as the Uredo twin's
//! `tests/api.ure`, so "they do the same thing" is a test result rather than a claim.
...
async fn start_stoppable() -> (u16, Arc<Notify>) {
    ...
    tokio::spawn(async move {
        let _ = restdemo_idiomatic::serve_until(listener, state, serve_stop).await;
    });
